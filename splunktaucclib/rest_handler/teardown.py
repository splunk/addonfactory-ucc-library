...
class TeardownHandler(base.BaseRestHandler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert hasattr(self, "targets") and self.targets, \
            RH_Err.ctl(1002,
                       msgx='{}.targets'.format(self._getHandlerName()),
                       shouldPrint=False)
        assert hasattr(self, "getArgs") and ismethod(self.getArgs), \
            RH_Err.ctl(1002,
                       msgx='%s.getArgs' % (self._getHandlerName()),
                       shouldPrint=False)
        assert hasattr(self, "distinguish") and ismethod(self.distinguish), \
            RH_Err.ctl(1002,
                       msgx='%s.distinguish' % (self._getHandlerName()),
                       shouldPrint=False)

        if self.requestedAction != admin.ACTION_LIST:
            RH_Err.ctl(1101,
                       msgx='Only "list" action is supported',
    ...
    def clean(self, endpoint):
        ...
        for ent in ents:
            ...
            resp, cont = splunkd_request(url_ent,
                                         self.getSessionKey(),
                                         method='DELETE',
                                         data={'output_mode': 'json'},
                                         retry=3
